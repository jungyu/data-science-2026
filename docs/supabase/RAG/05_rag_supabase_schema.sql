...
--
-- Execution order: extensions → functions → tables → indexes →
...
-- ULID 生成器（pk-convention: 所有業務表 PK 必須用 ULID）
-- 使用 gen_random_bytes() 取代 random()，確保密碼學等級隨機性
...
BEGIN
  ...
  FOR i IN REVERSE 9..0 LOOP
    ...
    unix_ts := unix_ts >> 5;
  END LOOP;
  -- Encode 80-bit random (10 bytes → 16 chars, but ULID spec = 10 chars random)
...
INSERT INTO public.embedding_models (name, provider, dimensions, max_tokens, description)
VALUES
...
CREATE TABLE IF NOT EXISTS public.collections (
  ...
  embedding_model_id TEXT       NOT NULL REFERENCES public.embedding_models(id),
...
CREATE TABLE IF NOT EXISTS public.chunk_tags (
...
CREATE TABLE IF NOT EXISTS public.query_logs (
  ...
  -- Agentic RAG 追蹤
  session_id      TEXT,
  parent_query_id TEXT REFERENCES public.query_logs(id),
...
-- ============================================================
CREATE TABLE IF NOT EXISTS public.query_log_results (
  id          TEXT PRIMARY KEY DEFAULT generate_ulid(),
  query_id    TEXT        NOT NULL REFERENCES public.query_logs(id) ON DELETE CASCADE,
  chunk_id    TEXT        NOT NULL REFERENCES public.chunks(id) ON DELETE CASCADE,
...
CREATE INDEX IF NOT EXISTS idx_collections_code ON public.collections(code);
CREATE INDEX IF NOT EXISTS idx_collections_embedding_model ON public.collections(embedding_model_id);
CREATE INDEX IF NOT EXISTS idx_collections_owner ON public.collections(owner_id) WHERE owner_id IS NOT NULL;

CREATE INDEX IF NOT EXISTS idx_documents_collection ON public.documents(collection_id);
CREATE INDEX IF NOT EXISTS idx_documents_status ON public.documents(process_status);
CREATE INDEX IF NOT EXISTS idx_documents_hash ON public.documents(content_hash) WHERE content_hash IS NOT NULL;
CREATE INDEX IF NOT EXISTS idx_documents_source_ref ON public.documents(source_ref_type, source_ref_id) WHERE source_ref_type IS NOT NULL;
CREATE INDEX IF NOT EXISTS idx_documents_owner ON public.documents(owner_id) WHERE owner_id IS NOT NULL;
CREATE INDEX IF NOT EXISTS idx_documents_lang ON public.documents(lang) WHERE lang IS NOT NULL;

CREATE INDEX IF NOT EXISTS idx_chunks_document ON public.chunks(document_id);
CREATE INDEX IF NOT EXISTS idx_chunks_collection ON public.chunks(collection_id);
CREATE INDEX IF NOT EXISTS idx_chunks_owner ON public.chunks(owner_id) WHERE owner_id IS NOT NULL;
...
CREATE INDEX IF NOT EXISTS idx_chunk_tags_tag ON public.chunk_tags(tag_id);

CREATE INDEX IF NOT EXISTS idx_query_logs_collection ON public.query_logs(collection_id, created_at DESC);
CREATE INDEX IF NOT EXISTS idx_query_logs_session ON public.query_logs(session_id) WHERE session_id IS NOT NULL;
CREATE INDEX IF NOT EXISTS idx_query_logs_parent ON public.query_logs(parent_query_id) WHERE parent_query_id IS NOT NULL;

CREATE INDEX IF NOT EXISTS idx_query_log_results_query ON public.query_log_results(query_id);
CREATE INDEX IF NOT EXISTS idx_query_log_results_chunk ON public.query_log_results(chunk_id);
CREATE INDEX IF NOT EXISTS idx_query_log_results_chunk_score ON public.query_log_results(chunk_id, score DESC);
...
CREATE OR REPLACE FUNCTION public.sync_document_owner()
RETURNS TRIGGER
LANGUAGE plpgsql
SECURITY DEFINER
SET search_path = public
...
CREATE TRIGGER trg_documents_sync_owner
  BEFORE INSERT OR UPDATE OF collection_id ON public.documents
  FOR EACH ROW EXECUTE FUNCTION public.sync_document_owner();

-- 強制 chunks.collection_id 與 document 一致 + 傳播 owner_id
CREATE OR REPLACE FUNCTION public.sync_chunk_from_document()
RETURNS TRIGGER
LANGUAGE plpgsql
SECURITY DEFINER
SET search_path = public
...
CREATE TRIGGER trg_chunks_sync_collection
  BEFORE INSERT OR UPDATE OF document_id ON public.chunks
  FOR EACH ROW EXECUTE FUNCTION public.sync_chunk_from_document();
...
CREATE OR REPLACE FUNCTION public.is_collection_owner(p_collection_id TEXT)
RETURNS BOOLEAN
LANGUAGE SQL
STABLE
SECURITY DEFINER
...
CREATE OR REPLACE FUNCTION public.is_collection_active(p_collection_id TEXT)
RETURNS BOOLEAN
LANGUAGE SQL
STABLE
SECURITY DEFINER
...
AS $$
  SELECT COALESCE(
    (SELECT is_active FROM public.collections WHERE id = p_collection_id),
    FALSE
  );
$$;

CREATE OR REPLACE FUNCTION public.can_read_collection(p_collection_id TEXT)
RETURNS BOOLEAN
LANGUAGE SQL
STABLE
SECURITY DEFINER
...
CREATE OR REPLACE FUNCTION public.match_chunks_with_document(
  query_embedding vector(1536),
  p_collection_id TEXT,
  p_top_k INTEGER DEFAULT 5,
  p_similarity_threshold FLOAT8 DEFAULT 0.7
...
CREATE OR REPLACE FUNCTION public.collection_stats(p_collection_id TEXT)
RETURNS TABLE (
...
AS $$
  SELECT
    (SELECT count(*) FROM public.documents WHERE collection_id = p_collection_id),
    (SELECT count(*) FROM public.chunks WHERE collection_id = p_collection_id),
    (SELECT count(*) FROM public.chunks WHERE collection_id = p_collection_id AND embedding IS NOT NULL),
    (SELECT avg(token_count) FROM public.chunks WHERE collection_id = p_collection_id),
    (SELECT count(*) FROM public.query_logs WHERE collection_id = p_collection_id),
    (SELECT avg(eval_faithfulness) FROM public.query_logs WHERE collection_id = p_collection_id AND eval_faithfulness IS NOT NULL);
$$;

CREATE OR REPLACE FUNCTION public.top_hit_chunks(
  p_collection_id TEXT,
...
AS $$
  ...
  FROM public.query_log_results r
  JOIN public.chunks c ON c.id = r.chunk_id
  WHERE c.collection_id = p_collection_id
...
ALTER TABLE public.chunk_tags ENABLE ROW LEVEL SECURITY;
ALTER TABLE public.query_logs ENABLE ROW LEVEL SECURITY;
ALTER TABLE public.query_log_results ENABLE ROW LEVEL SECURITY;
...
-- embedding_models
CREATE POLICY "embedding_models_read" ON public.embedding_models
  FOR SELECT TO authenticated, anon USING (TRUE);
CREATE POLICY "embedding_models_service" ON public.embedding_models
  FOR ALL TO service_role USING (TRUE) WITH CHECK (TRUE);

-- collections
CREATE POLICY "collections_read" ON public.collections
  FOR SELECT TO authenticated, anon
  USING (is_active = TRUE OR owner_id = public.get_current_owner_id());
...
CREATE POLICY "collections_delete" ON public.collections
  ...
  USING (owner_id = public.get_current_owner_id());
CREATE POLICY "collections_service" ON public.collections
  FOR ALL TO service_role USING (TRUE) WITH CHECK (TRUE);

-- documents
CREATE POLICY "documents_read" ON public.documents
  FOR SELECT TO authenticated, anon
  USING (owner_id = public.get_current_owner_id() OR public.is_collection_active(collection_id));
...
CREATE POLICY "chunks_write" ON public.chunks
  FOR INSERT TO authenticated
  WITH CHECK (public.is_collection_owner(collection_id));
CREATE POLICY "chunks_update" ON public.chunks
  FOR UPDATE TO authenticated
  USING (owner_id = public.get_current_owner_id());
CREATE POLICY "chunks_delete" ON public.chunks
  FOR DELETE TO authenticated
  USING (owner_id = public.get_current_owner_id());
CREATE POLICY "chunks_service" ON public.chunks
  FOR ALL TO service_role USING (TRUE) WITH CHECK (TRUE);

-- tags
CREATE POLICY "tags_read" ON public.tags
  FOR SELECT TO authenticated, anon USING (TRUE);
CREATE POLICY "tags_service" ON public.tags
  FOR ALL TO service_role USING (TRUE) WITH CHECK (TRUE);

CREATE POLICY "chunk_tags_read" ON public.chunk_tags
  FOR SELECT TO authenticated, anon USING (TRUE);
CREATE POLICY "chunk_tags_service" ON public.chunk_tags
  FOR ALL TO service_role USING (TRUE) WITH CHECK (TRUE);

-- query_logs
CREATE POLICY "query_logs_read" ON public.query_logs
  FOR SELECT TO authenticated
  USING (public.can_read_collection(collection_id));
CREATE POLICY "query_logs_insert" ON public.query_logs
  FOR INSERT TO authenticated
  WITH CHECK (public.can_read_collection(collection_id));
CREATE POLICY "query_logs_service" ON public.query_logs
  FOR ALL TO service_role USING (TRUE) WITH CHECK (TRUE);

-- query_log_results
CREATE POLICY "query_log_results_read" ON public.query_log_results
  FOR SELECT TO authenticated
  USING (
    public.can_read_collection(
      (SELECT collection_id FROM public.query_logs WHERE id = query_log_results.query_id)
    )
  );
CREATE POLICY "query_log_results_insert" ON public.query_log_results
  FOR INSERT TO authenticated
  WITH CHECK (
    public.can_read_collection(
      (SELECT collection_id FROM public.query_logs WHERE id = query_log_results.query_id)
...
CREATE POLICY "query_log_results_service" ON public.query_log_results
  FOR ALL TO service_role USING (TRUE) WITH CHECK (TRUE);
...
GRANT SELECT ON public.embedding_models TO authenticated, anon;
...
GRANT SELECT ON public.chunk_tags TO authenticated, anon;
GRANT INSERT, UPDATE, DELETE ON public.chunk_tags TO authenticated;
GRANT ALL ON public.chunk_tags TO service_role;

GRANT SELECT ON public.query_logs TO authenticated;
GRANT INSERT, UPDATE ON public.query_logs TO authenticated;
GRANT ALL ON public.query_logs TO service_role;

GRANT SELECT ON public.query_log_results TO authenticated;
GRANT INSERT ON public.query_log_results TO authenticated;
GRANT ALL ON public.query_log_results TO service_role;

-- Helper functions
GRANT EXECUTE ON FUNCTION public.generate_ulid() TO authenticated, anon, service_role;
GRANT EXECUTE ON FUNCTION public.get_current_owner_id() TO authenticated;
GRANT EXECUTE ON FUNCTION public.is_collection_owner(TEXT) TO authenticated;
GRANT EXECUTE ON FUNCTION public.is_collection_active(TEXT) TO authenticated, anon;
GRANT EXECUTE ON FUNCTION public.can_read_collection(TEXT) TO authenticated, anon;
GRANT EXECUTE ON FUNCTION public.match_chunks(vector, TEXT, INTEGER, FLOAT8) TO authenticated, anon;
GRANT EXECUTE ON FUNCTION public.match_chunks_with_document(vector, TEXT, INTEGER, FLOAT8) TO authenticated, anon;
GRANT EXECUTE ON FUNCTION public.hybrid_search(TEXT, vector, TEXT, INTEGER, FLOAT8, FLOAT8) TO authenticated, anon;
GRANT EXECUTE ON FUNCTION public.collection_stats(TEXT) TO authenticated;
GRANT EXECUTE ON FUNCTION public.top_hit_chunks(TEXT, INTEGER) TO authenticated;


-- ============================================================
-- 16. DEVELOPMENT HELPER（部署前移除）
-- ============================================================
-- CREATE POLICY "dev_all" ON public.collections FOR ALL USING (TRUE);
-- CREATE POLICY "dev_all" ON public.documents FOR ALL USING (TRUE);
-- CREATE POLICY "dev_all" ON public.chunks FOR ALL USING (TRUE);
-- CREATE POLICY "dev_all" ON public.query_logs FOR ALL USING (TRUE);
-- CREATE POLICY "dev_all" ON public.query_log_results FOR ALL USING (TRUE);
